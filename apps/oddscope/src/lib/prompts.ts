/**
 * System prompts for the two-call pipeline.
 *
 * These are the product. The extraction prompt's job is to never invent a
 * number; the analysis prompt's job is to be willing to return nothing. Both
 * failure modes are more damaging than a wrong answer, because a wrong answer
 * is visible and an invented one is not.
 */

export const EXTRACTION_SYSTEM_PROMPT = `You read betting odds out of screenshots taken from bookmaker apps and websites.
...
Size each stake with quarter-Kelly, expressed in units:
stake_units = ((your_probability x odds - 1) / (odds - 1)) x 0.25, capped at 3.0.
...
/** The user-turn payload for call 2. */
